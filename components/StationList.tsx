...
export default function StationList({ stations, selectedId, onSelect, fuel, favorites = [], mobile = false, onClose }: Props) {
  ...
  return (
    <div className={mobile ? 'flex flex-col h-full overflow-hidden' : 'w-72 flex flex-col rounded-2xl bg-[var(--panel)] border border-[var(--panel-border)] shadow-xl backdrop-blur-md overflow-hidden'}>
      {/* Header */}
      <div
        className={`flex items-center justify-between px-3 py-2 border-b border-[var(--panel-border)] ${mobile ? '' : 'cursor-pointer select-none'}`}
        onClick={() => !mobile && setCollapsed(c => !c)}
      >
